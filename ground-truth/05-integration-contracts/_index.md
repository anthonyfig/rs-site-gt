...
related: ["gt-03-capability-lead-capture", "gt-04-stack-and-architecture", "gt-02-business-rules"]
...
| **Slack** | **Real-time notifications** on new lead & booked call (via HubSpot) | — (notification only; not a source of truth) | HubSpot → Slack (event) | HubSpot↔Slack integration / webhook | Must **not** block lead creation/booking if Slack is down |
...
- Detailed per-system contracts (endpoints, payloads, webhooks) are authored when each capability
  that needs them is built (next: `hubspot.md` for lead-capture).
