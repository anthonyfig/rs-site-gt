...
| **Sanity** | **Headless CMS** — marketing content editor (Studio) | **Case studies, blog/insights, page copy** | Sanity → Astro (read at build); webhook → Vercel rebuild | Read token (server/CI); write token for migration | Failed fetch fails the build, not the live site. Full contract → [sanity.md](sanity.md) · Decision 0013 |
| **Webflow** ("RS2.0 - MASTER") | **Retiring** — legacy host/CMS | — (migrating out) | n/a | — | Content migrates to **Sanity** (Decision 0013); Webflow decommissioned after cutover |
...
- Detailed per-system contracts (endpoints, payloads, webhooks) are authored when each capability
  ...
  [anthropic-api.md](anthropic-api.md) (Explorer "Ask the Ground Truth" chat), and [sanity.md](sanity.md) (marketing content CMS — Decision 0013).
